<template>
  <div class="search">
      ...
      <div class="shop" v-if="result && !showShop">
          ...
          <div v-else>
              <SearchIndex @click="showItemClick" :data="result.restaurants"/>
               <SearchIndex @click="showItemClick" :data="result.words"/>
          </div>
      </div>
...
export default {
    ...
    methods: {
      ...
      searchHandle(){
          if(!this.key_word) return false;
          if(this.result && (this.result.restaurants>0 || this.result.words.length)){
              console.log('有内容');
              this.showItemClick()
          }else{
              this.empty=true
